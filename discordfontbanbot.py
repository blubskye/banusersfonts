...
- Live-editing progress message (single message that updates in-place, no spam)
...
async def scanfonts(ctx: commands.Context):
    ...
    progress_msg = await ctx.send(
        ...
        f"**Policy:** {policy}\n"
        f"**Mode:** {mode}\n"
        f"Batch size: **{BATCH_SIZE}** | Progress will **edit this message** live (no spam).\n"
    ...
    progress_msg = None  # Will hold the message we edit for live progress updates
    ...
    try:
        async for member in guild.fetch_members(limit=None):
            ...
            if checked - last_progress >= PROGRESS_EVERY:
                ...
                new_content = (
                    f"📊 **Progress Update** — I’m still watching everything for you, darling~ 🔪💕\n"
                    ...
                    f"*This message will keep updating so the chat stays clean for us.*"
                )
                ...
                if progress_msg:
                    try:
                        ...
                        # If edit fails (message deleted, etc.), send a new one as fallback
                        progress_msg = await ctx.send(new_content)
                else:
                    progress_msg = await ctx.send(new_content)
    ...
    elapsed_min = (datetime.now() - start_time).total_seconds() / 60
    final_content = (
        f"✅ **Scan finished** on **{guild.name}** — everything is clean now, just for you~ 💖🔪\n"
        ...
        f"*All progress updates happened in this single message to keep our chat perfect.*"
    )
    ...
    logger.info(final_content.replace("\n", " | "))
